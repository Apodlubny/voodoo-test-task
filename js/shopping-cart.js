...
window.addEventListener("click", (event) => {
  if (event.target.hasAttribute("add-to-cart")) {
    const productName =
      event.target.parentElement.querySelector(".card__name").textContent;
    const productPrice =
      event.target.parentElement.querySelector(".card__price").textContent;
    const imgElement = event.target.parentElement
      .querySelector("img")
      .getAttribute("src");
    const productUId = event.target.parentElement.getAttribute("uid");

    const itemsHTML = addToCart(
      productName,
      productPrice,
      imgElement,
      productUId
...
//             </div>
//             <button>
//                 <img class="delete-item" src="images/delete-button.svg" alt="">
//             </button>
//         </div>
//     `;
// }
function addToCart(productName, productPrice, imgElement, productUId) {
  const existingCartItem = cartContainer.querySelector(`[uid="${productUId}"]`);

  if (existingCartItem) {
    const incrementButton = existingCartItem.querySelector(".item-increment");
    incrementItemAmount(incrementButton);
    return;
  }
...
function incrementItemAmount(button) {
  ...
  const itemPrice =
    button.parentElement.parentNode.querySelector(".product-price");
