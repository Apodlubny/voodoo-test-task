...
function generateProductHTML(product) {
  let imageSrc = "images/product-photo.png";
...
function generatePaginationLinks(totalPages, currentPage) {
  ...
  if (minPage > 1) {
    ...
    if (minPage > 2) {
      paginationHTML.push(
        `<a href="?page=${minPage - 1}" class="pagination__page">...</a>`
      );
    }
  }
  ...
  for (let i = minPage; i <= maxPage; i++) {
    if (i === currentPage) {
      paginationHTML.push(
        `<a href="?page=${i}" class="pagination__page pagination__page-current">${i}</a>`
      );
    } else {
      paginationHTML.push(
  ...
  if (maxPage < totalPages) {
    if (maxPage < totalPages - 1) {
      paginationHTML.push(
        `<a href="?page=${maxPage + 1}" class="pagination__page">...</a>`
      );
    }
    paginationHTML.push(
      `<a href="?page=${totalPages}" class="pagination__page">${totalPages}</a>`
    );
  }
